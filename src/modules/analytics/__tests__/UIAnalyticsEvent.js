...
it('should construct with required args', () => {
  ...
  expect(analyticsEvent.handlers.length).toBe(1);
  expect(analyticsEvent.handlers[0].toString()).toEqual(noop.toString());
});
...
it('can be cloned into a new event', () => {
  ...
  expect(analyticsEvent.action).toEqual(newEvent.action);
